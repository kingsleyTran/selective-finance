<header class="fixed top-0 left-0 right-0 z-50 w-full transition-colors duration-300"
    ...
    <div class="mx-auto max-w-screen-2xl px-4 md:px-6">
        <div
            class="flex h-16 items-center justify-between lg:my-6 lg:h-auto"
            :class="scrolled ? 'lg:mt-4' : ''">

            {{-- Logo --}}
            <a href="/" class="flex shrink-0 items-center" @click="mobileOpen = false">
                <img src="/images/logo.svg" alt="Selective Finance" class="h-10 w-auto lg:h-20">
            </a>

            {{-- Desktop menu --}}
            <div class="hidden flex-row items-center gap-2 lg:flex">
                <nav
                    class="flex items-center gap-8 rounded-full border border-white/20 bg-white/10 px-10 py-3">
            ...
            {{-- Mobile menu button --}}
            <button type="button"
                class="flex h-10 w-10 items-center justify-center rounded-lg text-white transition hover:bg-white/10 lg:hidden"
